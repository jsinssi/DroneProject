...
def main():
    ...
    while True:
        ...
        # 3. Rectangles
        rects_img = detect_parking_rectangles(gray, marker_boxes)

        # 4. Assign bays whenever we have any pose estimate
        if H_used is not None:
            for (x, y, w, h) in rects_img:
                cx = x + w / 2.0
                cy = y + h / 2.0
                ...
                if bay_id is not None:
                    cv2.putText(frame, f"Bay {bay_id}",
                                (x, y - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                                (0, 0, 0), 2, cv2.LINE_AA)  # changed to black

                cv2.putText(frame, f"{int(wx)},{int(wy)}",
                            (x, y + h + 15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (0, 255, 0), 1, cv2.LINE_AA)
        else:
            # Pose not initialised yet – just show rectangles
            for (x, y, w, h) in rects_img:
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)

        # HUD
        cv2.putText(frame, f"Unique bays seen: {len(seen_bays)}",
                    (20, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                    (255, 255, 255), 2, cv2.LINE_AA)

        cv2.putText(frame, f"Rects this frame: {len(rects_img)}",
                    (20, 60),
